...
<!-- heading loop-->
<div v-for="(item, index1) in props.data[0][1]" class="mt-1 flex flex-col">
    <div class="flex flex-row">
        <!-- main heading number -->
        <div class="justify-end w-10 h-[16px] flex items-center">{{ index+1 }}</div>
        <div class="h-[16px] w-10 flex items-center">{{ '.'+(index1+1) }}</div>
        <button @click.prevent="IndexSubHeadingOpen[props.index] = !IndexSubHeadingOpen[props.index]" class="h-[16px] flex items-center hover:text-lime-600 font-bold text-sm" type="button">{{ props.data[index][1][index1][0] }}</button>
        ...
        <div class="text-xs w-fit h-[16px] flex items-center">
            <div class="">{{ typeof props.data[index][1][index1][1] != 'undefined' ? props.data[props.index][1][1].length : '' }}</div>
