<template>
    ...
    <div class="flex flex-col">
        ...
        <div aria-label="Header" :class="{'mb-14': 1, 'mb-5': tagQuickFilterBarOpen == 0, 'mb-10': !props.home}" class="flex items-center xl:justify-between xl:items-end mx-2 xl:mx-10">
                ...
                <div aria-label="search box" class="xl:relative xl:w-full flex xl:justify-end w-full items-center">
                    ...
                    <div v-if="1" aria-label="quickbar" class="absolute flex left-0 top-[53px] xl:top-12 xl:gap-3 gap-1 w-full justify-center xl:justify-start text-sm xl:text-lg">
                        <div v-if="props?.home?.story" class="flex items-center">
                            ...
                            <!-- <Link :href="route('filter', {'category': 'story'})" class="underline flex flex-row"><b>Story</b><span class="hidden 2xl:block">{{props?.home?.story?.[0] ? '(' + props?.home?.story?.[0] + ')' : ''}} </span></Link> -->
                            <Link :href="route('filter', {'category': 'story'})" class="underline flex flex-row"><b>Story</b><span class="hidden 2xl:block"></span></Link>

                        </div>
                        <div v-if="data?.fact" class="flex items-center">
...
// if (typeof props.home != undefined) {
    // const form3 = useForm('key2', props.home);
// }

onMounted(() => {
    // console.log(props.dataParent)

    // if (typeof props?.home?.length > 0) {
    //     console.log('ok');
    ...
    // console.log(props?.home?.fact?.[0]);

    // let test1234 = props?.home?.fact?.[0];
    // console.log(test1234);

    // Inertia.remember({'test': test1234}, 'key99');
...
watch(() => props.home, (curr, prev) => {
    ...
    // if (!Inertia.restore('key999')) {
        // Inertia.remember({'test': 123}, 'key999');
    // }

    // const form55 = useForm('CreateUser', {'test': 12345})
